...
- Layout: `workspace.py` all agent tools (shared by MCP server, GUI and benchmark) · `agent.py` Agent SDK runner with metrics · `app.py` + `static/` GUI (`app.js` view/tree/agent panel, `sketch.js` sketch editor, `icons.js`) · `bench.py` agent benchmark · `macros.py` sketch shortcut ops · `schema.py` IR models · `expr.py` params/units · `sketch.py` PlaneGCS + regions · `frames.py` plane conventions · `topo.py` face labels and FaceRef/EdgeRef resolution · `features.py` feature builders · `regen.py` regeneration + cache · `ops.py` edit ops · `session.py` undo/history/autosave · `mcp_server.py` agent tools · `render.py` PNGs · `cli.py`.
